...
class PurchaseControllerIT extends AbstractContainerTest {
  ...
  void givenValidRequestBodyWhenSavePurchaseThenReturnResponse() throws JsonProcessingException {
    ...
    final String token = Authenticator.admin();
  ...
  void givenNonexistentPurchaseWhenSaveThenReturnEntityNotFound() {
    final var response = given()
        ...
        .statusCode(404)
        .body(
            JsonSchemaValidator.matchesJsonSchemaInClasspath("jsons/schemas/violationSchema.json"))
        .extract()
        .as(ErrorResponseDto.class);
